...
func (s *Store) Connect() error {
	...
	if err := s.client.Ping(s.ctx, readpref.Primary()); err != nil {
		return errors.Wrap(err, "mongodb ping error")
	}
...
// GetAutoIncId 生成自增ID
func (s *Store) GetAutoIncId(ctx context.Context, id string) (int64, error) {
	return GetAutoIncId(ctx, s.D(), id)
}
...
func (s *Store) Scan(dbName, tabName string, cur, size int64, filter interface{}, result interface{}, fn ...func(opts *options.FindOptions) *options.FindOptions) *Scan {
	...
	_ = s.Client().UseSession(ctx, func(sctx mongo.SessionContext) error {
