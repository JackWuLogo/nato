...
func (c *Client) Unmarshal(raw []byte) (head *ClientHead, data []byte, err error) {
	var rawLen = len(raw)
	if rawLen < c.headLen {
		return nil, nil, errors.DataInvalid("msg head length error")
	}
	...
	if c.mixLen > 0 {
		...
		for i, head := range heads {
			if c.mixHead[i] != head {
				return nil, nil, errors.DataInvalid("msg head check error")
			}
		}
	}
	...
	if rawLen > c.headLen {
		...
		if rawLen < maxLen {
			return nil, nil, errors.DataInvalid("msg data length error")
		}
