		...
		for i, fn := range w.worker {
			...
			go func(i int, handler WorkHandler) {
				...
				if isTrace {
					since := time.Since(st)
					if err != nil {
						trace = append(trace, color.Error.Text("\t-> run %d work uptime: %s, error: %v", i, since, err))
					} else if since > time.Second {
						trace = append(trace, color.Question.Text("\t-> run %d task uptime too long: %s", i, since))
					}
				}
			}(i, fn)
		}
