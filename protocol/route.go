...
func (h *Route) Call(gmt *gateway.Meta, req, rsp interface{}) error {
...
func parseProtocolCmd(name string) (uint32, error) {
	...
	if len(names) != 2 {
		return 0, errors.Server("%s.%s format error", name, name)
	}

	cmd, err := strconv.ParseUint(names[1], 10, 32)
	if err != nil {
		return 0, errors.Server("%s.%s invalid command: %s", name, name, names[1])
	}
